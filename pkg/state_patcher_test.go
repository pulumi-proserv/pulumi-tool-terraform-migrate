...
// buildV2Providers creates the providers map and pulumiToTFType map needed by PatchStateV2
...
func buildV2Providers(t *testing.T, prov *ProviderWithMetadata, tokenOverrides map[string]string) (
...
func TestPatchStateV2_PatchesFromDigest(t *testing.T) {
	t.Parallel()
...
func TestPatchStateV2_DefaultFallback(t *testing.T) {
	t.Parallel()
...
func TestPatchStateV2_AssetFromSchema(t *testing.T) {
	t.Parallel()
	...
	patched, result, err := PatchStateV2(stateData, digest, providers, typeMap, nil, resourceMappings, nil, tmpDir)
	require.NoError(t, err)
	assert.Equal(t, 1, result.Patched)
	assert.Equal(t, 1, result.FieldsFromDigest)
...
func TestPatchStateV2_SensitiveResolution(t *testing.T) {
	t.Parallel()
	...
	patched, result, err := PatchStateV2(stateData, digest, providers, typeMap, nil, resourceMappings, configSecrets, "")
	require.NoError(t, err)
	assert.Equal(t, 1, result.Patched)
	assert.Equal(t, 1, result.FieldsFromDigest)  // resolved from config
	assert.Equal(t, 0, result.SkippedSensitive)   // none skipped
