...
func ResolveProperties(
	...
	for k, v := range props {
		rv, err := resolveValue(ctx, v, resources, resourceTypes, exports, cc, true)
		if err != nil {
...
// instead of resolved.
func resolveValue(ctx context.Context, v interface{}, resources, resourceTypes, exports map[string]string, cc CloudControlReader, topLevel bool) (interface{}, error) {
	switch val := v.(type) {
	case map[string]interface{}:
		...
		if len(val) == 1 {
			...
			if joinArgs, ok := val["Fn::Join"].([]interface{}); ok && len(joinArgs) == 2 {
				...
				for _, item := range list {
					rv, err := resolveValue(ctx, item, resources, resourceTypes, exports, cc, false)
					if err != nil {
						return "", err
					}
			...
			// (Fn::Sub, Fn::Select, ...) — surface it rather than pass a raw
		...
		for k, vv := range val {
			rv, err := resolveValue(ctx, vv, resources, resourceTypes, exports, cc, false)
			if err != nil {
				return nil, err
			}
			out[k] = rv
